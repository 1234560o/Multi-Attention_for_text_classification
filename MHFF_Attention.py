...
class Text_attention(object):
    def __init__(self,
        ...
        with tf.variable_scope("FFA_output"):
            self.attention_vec = tf.get_variable("attention_vec",
                                                 shape=[self.sent_attention_size],
                                                 initializer=create_initializer(self.initializer_range))
            sent_vec = tf.layers.dense(
                tf.expand_dims(self.attention_vec, axis=[0]),
                self.hidden_size,
                activation=tf.tanh,
                name="FFA",
                kernel_initializer=create_initializer(self.initializer_range))
            sent_vec = tf.reshape(sent_vec, [self.hidden_size]) # [H]
            # 最后一个维度的元素相乘再在最后一个维度相加
            atten_coeff = tf.reduce_sum(tf.multiply(self.final_outputs[-1], sent_vec), axis=-1)  # [B, S]
            # 对padding部分的位置做一个mask
            atten_adder = (1.0 - tf.cast(self.input_mask, tf.float32)) * -10000.0
            atten_coeff += atten_adder
            self.attention_coeff = tf.nn.softmax(atten_coeff)  # [B, S]
            self.pooled_out = tf.reduce_sum(self.final_outputs[-1] * tf.expand_dims(self.attention_coeff, axis=[-1]), axis=1)
            fc1 = tf.layers.dense(self.pooled_out, self.hidden_size, activation=gelu,
                                  kernel_initializer=create_initializer(initializer_range))
            self.scores = tf.layers.dense(fc1, self.num_classes,
                                          kernel_initializer=create_initializer(initializer_range))
...
if __name__ == "__main__":
    ...
    with tf.Session() as sess:
        ...
        a, b = sess.run([model.acc, model.loss], feed_dict={model.input_x: input_x,
